using NUnit.Framework;
using ZERO01.Data.Entity.AppCustomer;
using ZERO01.Data.Entity.AppProduct;
using ZERO01.Data.Repository;
using ZERO01.Shared.Helpers;
using ZERO01.Shared.Pagination;
using System;
using System.Collections.Generic;

namespace ZERO01.DataTests.Tests
{
    [TestFixture]
    public class CustomerRepositoryTest
    {
        private Customer GenerateInput(bool generateId = false, int? id = null)
        {
            var input = new Customer
            {
                Name = nameof(Customer) + Helper.RandomString(6),
                PhoneNumber = Helper.RandomNumber(10),
                AccumulatedPoint = 0,
                CustomerLevelId = 1,
                CreationTime = DateTime.Now
            };
            if (generateId) input.Id = 111_111_111;
            if (id != null) input.Id = (int)id;
            return input;
        }
        
        private bool CompareProperties(Customer expected, Customer actual)
        {
            return 
            (
                expected.Name == actual.Name &&
                expected.AccumulatedPoint == actual.AccumulatedPoint &&
                expected.CustomerLevelId == actual.CustomerLevelId
            );
        }

        // Basic
        [Test]
        public void Get_Success_ReturnEntity()
        {
            // Arrange
            var repository = new CustomerRepository();
            var input = repository.Create(GenerateInput());

            // Act
            var result = repository.Get(input.Id);

            // Assert
            Assert.IsInstanceOf<Customer>(result);
        }

        [Test]
        public void Get_Fail_ReturnNull()
        {
            // Arrange
            var repository = new CustomerRepository();

            // Act
            var result = repository.Get(111_111_111);

            // Assert
            Assert.IsNull(result);
        }

        [Test]
        public void Create_Success_ReturnEntity()
        {
            // Arrange
            var repository = new CustomerRepository();
            var input = GenerateInput();

            // Act
            var result = repository.Create(input);

            // Assert
            Assert.That(CompareProperties(input, result));
        }

        [Test]
        public void Update_Success_ReturnTrue()
        {
            // Arrange
            var repository = new CustomerRepository();
            var input = repository.Create(GenerateInput());

            var inputForUpdate = GenerateInput(id: input.Id);

            // Act
            var result = repository.Update(inputForUpdate);

            // Assert
            Assert.IsTrue(result);
        }

        [Test]
        public void Update_Success_ReturnFalse()
        {
            // Arrange
            var repository = new CustomerRepository();
            var input = GenerateInput(generateId: true);

            // Act
            var result = repository.Update(input);

            // Assert
            Assert.IsFalse(result);
        }

        [Test]
        public void Delete_Success_ReturnNull()
        {
            // Arrange
            var repository = new CustomerRepository();
            var input = repository.Create(GenerateInput());

            // Act
            repository.Delete(input.Id);
            var result = repository.Get(input.Id);

            // Assert
            Assert.IsNull(result);
        }

        // Additional
        [Test]
        public void GetCustomByPhoneNumber_Success_ReturnEntity()
        {
            // Arrange
            var repository = new CustomerRepository();
            var input = repository.Create(GenerateInput());

            // Act
            var result = repository.GetCustomByPhoneNumber(input.PhoneNumber);

            // Assert
            Assert.IsInstanceOf<Customer>(result);
        }

        [Test]
        public void GetAllCustomers_Success_ReturnEntities()
        {
            // Arrange
            var repository = new CustomerRepository();

            // Act
            var result = repository.GetAllCustomers();

            // Assert
            Assert.IsInstanceOf<IEnumerable<Customer>>(result);
        }

        [Test]
        public void GetCustomers_Success_ReturnEntities()
        {
            // Arrange
            var repository = new CustomerRepository();

            // Act
            var result = repository.GetAllCustomers();

            // Assert
            Assert.IsInstanceOf<IEnumerable<Customer>>(result);
        }
    }
}
